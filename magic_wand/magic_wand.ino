...
void loop() {
  ...
  bool got_data = ReadAccelerometer(error_reporter, model_input->data.f,
                                    input_length, should_clear_buffer, mpu.getAcc(0), mpu.getAcc(1), mpu.getAcc(2));
  ...
  Serial.println("After condition Magic");
...
void configureIMU() {
  if (!mpu.setup(0x69)) {  // change to your own address
    ...
    mpu.setAccBias(29.36, 11.22, 55.74);
    mpu.setGyroBias(0.07, 2.55, -0.74); 
}
